...
class ContactNetwork():
	...
	def init_Su_In_Re(self):
		...
		infected_recovered[
			...
			round(self.fraction_infected*self.n + self.fraction_recovered*self.n)
			] = 2
		np.random.shuffle(infected_recovered)
		infected = np.array(
			[1. if i == 1 else 0. for i in infected_recovered]).reshape(self.n, 1)
		recovered = np.array(
			[1. if i == 2 else 0. for i in infected_recovered]).reshape(self.n, 1)
